...
def triton_dynamic_quantize(out, input, scale):
    ...
    method_name = "dynamic_quant_" + str(hidden_size)
    kwargs = [
        out,
        input,
        scale,
    ...
    grid = (num_tokens, 1, 1)
    const_kwargs = {"N": hidden_size, "num_warps": num_warps}
    compile_and_cache_kernels(_triton_dynamic_quantize_kernel, method_name, grid, kwargs, const_kwargs)
