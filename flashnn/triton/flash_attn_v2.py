...
from flasnn.triton_kernels.triton_utils import get_autotune_triton_kernel, compile_and_cache_kernels
...
def _triton_attn_fwd(
    ...
    q_offset = off_z.to(tl.int64) * stride_qz + (off_h * GROUPS + off_g).to(tl.int64) * stride_qh
    k_offset = off_z.to(tl.int64) * stride_kz + off_h.to(tl.int64) * stride_kh
    v_offset = off_z.to(tl.int64) * stride_vz + off_h.to(tl.int64) * stride_vh
    o_offset = off_z.to(tl.int64) * stride_oz + (off_h * GROUPS + off_g).to(tl.int64) * stride_oh
    # block pointers
...
def flash_attention_forward(q, k, v, causal, sm_scale=None, ORDER_12=False):
    # layout constraints
    ...
    method_name = "flash_attn_v2_" + "_".join(str(value) for value in const_kwargs.values())

    if get_autotune_triton_kernel():

        def grid(META):
            return (triton.cdiv(seq_len, META["BLOCK_M"]), batch_size * num_heads_k, groups)
        ...
        base_config = {'BLOCK_M': BLOCK_M, 'BLOCK_N': BLOCK_N, "num_stages": num_stages, "num_warps": num_warps}
        grid = (triton.cdiv(seq_len, base_config["BLOCK_M"]), batch_size * num_heads_k, groups)
        const_kwargs.update(base_config)
        flash_attn = _triton_attn_fwd
    grid_kwargs = ["BLOCK_M"]
    compile_and_cache_kernels(flash_attn, method_name, grid, kwargs, const_kwargs=const_kwargs, grid_kwargs=grid_kwargs)
    return o
